...
##다른 카피와는 다르게 deepcopy는 nested list 를 완벽하게 복사해내는등 거의 완벽한 복사를 자랑하는 모듈

##import random
##from copy import copy, deepcopy
#### 카피라는 모듈에서 copy, deepcopy 를 둘다 뽑아온다.
EMPTY = 0
PLAYER_X = 1
PLAYER_O = 2
DRAW = 3
...
def printboard(state):
    cells = []
    for i in range(3):
        ## range(3) = [0,1,2]  으로 for 문을 0분터 2까지 돌림
        for j in range(3):
            ## range(3) = [0,1,2]  으로 for 문을 0분터 2까지 돌림
    ...
    for i in range(3):
        for j in range(3):
            if state[i][j] == EMPTY:
                return EMPTY
                ## 비었으면 비었다는상태 출력
    return DRAW  ## 위의 모든 리턴에 걸리지 않았다면 비김을 출력
    ...
    for i in range(3):
        for j in range(3):
            if state[i][j] == PLAYER_X:
                ## PLAYER_X 를 나타내는1의 개수를 센다
                countx += 1
            elif state[i][j] == PLAYER_O:
                ## PLAYER_O 를 나타내는 2의 개수를 센다
                counto += 1
    if countx == counto:
        ## 같으면 O 의 차례 1을 리턴
        return PLAYER_O
...
class Agent(object):
    def __init__(self, player, verbose=False, lossval=0, learning=True):
        ## __init__ 를 이용하여 객체를 만들때 항상 실행되야
        self.values = {}
        print("a_vales" + str(self.values))
        self.player = player
        print('a_player' + str(self.player))
        self.verbose = verbose
        print('a_verbose' + str(self.verbose))
        self.lossval = lossval
        print('a_lossval' + str(self.lossval))
        self.learning = learning
        print('a_learning' + str(self.learning))
        self.epsilon = 0.1
        self.alpha = 0.99
        self.prevstate = None
        print('a_prevstate' + str(self.prevstate))
        self.prevscore = 0
        print('a_prevscore' + str(self.prevscore))
        self.count = 0
        print('a_count' + str(self.count))
        enumstates(emptystate(), 0, self)

    def episode_over(self, winner):
        print('episode_over_prevstate' + str(self.prevstate))
        print('episode_over_prevscore' + str(self.prevscore))
    ...
    def backup(self, nextval):
        ...
        if self.prevstate != None and self.learning:
            print('before_values' + str(self.values))
            self.values[self.prevstate] += self.alpha * (nextval - self.prevscore)
            print('after_values' + str(self.values))
